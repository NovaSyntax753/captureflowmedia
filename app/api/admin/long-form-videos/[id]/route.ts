...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const response = await supabaseRequest(
      `long_form_videos?id=eq.${encodeURIComponent(id)}&select=id,title,description,youtube_url,thumbnail_url,display_order,created_at`,
      {
        method: "PATCH",
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  try {
    const { id } = await context.params;
    const response = await supabaseRequest(`long_form_videos?id=eq.${encodeURIComponent(id)}`, {
      method: "DELETE",
    });
    if (!response.ok) throw new Error(await response.text());
